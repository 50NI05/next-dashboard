import { PokemonsResponse, TopLevel } from "@/pokemons"
...
export async function generateMetadata({ params }:Props):Promise<Metadata> {
  try {
    const nameParams = await params
    const { id, name } = await getPokemon(`${nameParams.name}`)
...
const getPokemon = async(name: string): Promise<TopLevel> => {
  try {
    const pokemon = await fetch(`https://pokeapi.com/api/v2/pokemon/${name}`, {
      cache: 'force-cache',
      // next: {
...
export default async function PokemonPage({ params }: Props) {
  const nameParams = await params
  const pokemon = await getPokemon(`${nameParams.name}`);

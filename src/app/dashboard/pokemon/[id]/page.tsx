...
export async function generateMetadata({ params }:Props):Promise<Metadata> {
  try {
    ...
    const { id, name } = await getPokemon(idParams.id)
...
const getPokemon = async(id: string): Promise<TopLevel> => {
  try {
    const pokemon = await fetch(`https://pokeapi.com/api/v2/pokemon/${id}`, {
      cache: 'force-cache',
      // next: {
...
export default async function PokemonPage({ params }: Props) {
  const idParams = await params
  const pokemon = await getPokemon(idParams.id);
